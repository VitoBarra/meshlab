...
int FilterUnsharp::getPreConditions(const QAction* a) const
{
	...
	case FP_HARMONIC_FIELD_SCALAR_SELECTED: return MeshModel::MM_FACENUMBER | MeshModel::MM_VERTQUALITY ;
...
int FilterUnsharp::postCondition(const QAction* a) const
{
	...
	case FP_HARMONIC_FIELD_SCALAR_SELECTED: return MeshModel::MM_VERTQUALITY | MeshModel::MM_VERTCOLOR | MeshModel::MM_VERTCOORD;
...
//	}
	...
	case FP_HARMONIC_FIELD_SCALAR_SELECTED: {
	// TODO: This filter extends the quality scalar value over the selected area using the values on the unselected area as constraints
		md.mm()->updateDataMask(MeshModel::MM_FACEFACETOPO);
		CMeshO& m = md.mm()->cm;
		vcg::tri::Allocator<CMeshO>::CompactEveryVector(m);
		...
		case 0: // Color
		{
			...
			NoErrorOccurs = tri::Harmonic<CMeshO, FieldScalar>::ComputeScalarField(m, Rconstraints, Rfield);

			if (!NoErrorOccurs) {
				throw MLException("failed Harmonic computer scalar field for selected vertex");
			}
			NoErrorOccurs = tri::Harmonic<CMeshO, FieldScalar>::ComputeScalarField(m, Gconstraints, Gfield);

			if (!NoErrorOccurs) {
				throw MLException("failed Harmonic computer scalar field for selected vertex");
			}
			NoErrorOccurs = tri::Harmonic<CMeshO, FieldScalar>::ComputeScalarField(m, Bconstraints, Bfield);
		...
		case 2: // Quality
		{
			...
			bool ok =tri::Harmonic<CMeshO, FieldScalar>::ComputeScalarField(m, constraints, handle);
